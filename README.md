...
Serve an extracted web artifact with any HTTP server. Browser builds currently show the demo without native mesh pairing or pings; those are available on Android and desktop.

CI uses Rust 1.98.1, Java 25, the checked-in Gradle wrapper, and Android NDK 26.3.11579264. To reproduce native preparation, run `./kmp/gradlew -p kmp prepareNative` with Cargo available, then run the relevant Gradle task from `kmp/`: `:demo:desktopApp:packageDistributionForCurrentOS`, `:demo:androidApp:assembleDebug`, `:demo:webApp:jsBrowserDistribution`, or `:demo:webApp:wasmJsBrowserDistribution`. Android additionally requires `android-native` in the KMP devenv shell. Desktop installers must be built on their target operating system. Outside devenv, Gradle downloads its matching Node.js, Yarn, and Binaryen tools; devenv supplies these tools directly.
