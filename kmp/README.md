...
S2 supports several meshes in Rust and the CLI, but FFI and KMP remain single-mesh until S3. `createMesh` atomically refuses a second mesh and withdraws outstanding pairing tickets; `pair` refuses while already in a mesh, while `add` and `leaveMesh` require exactly one current mesh. The KMP pairing session cannot select among multiple groups.
