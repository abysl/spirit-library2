...
The Rust and CLI APIs accept mesh IDs, but creation and enrollment still refuse a second mesh until the next PR. FFI and KMP remain single-mesh until S3. `createMesh` refuses a second mesh and withdraws any fresh-device pairing ticket when it creates the first; `add` and `leaveMesh` require a single current mesh. Do not use the old KMP pairing session to select among multiple groups.
