...
A fresh receiver stays enrollable until it scans a ticket. Its first eligible enrollment attempt creates the requested mesh immediately before enrollment; later scans reuse it. A failed remote enrollment can leave a founder-only mesh. Independent meshes cannot merge: an existing member must scan a fresh receiver's ticket to admit it into their mesh.

S2 supports several meshes in Rust and the CLI, but FFI and KMP remain single-mesh until S3. `createMesh` atomically refuses a second mesh and withdraws any outstanding ticket when creating the first; `pair` refuses while enrolled, so the QR cannot offer an impossible second join. `add` and `leaveMesh` require exactly one current mesh; the KMP pairing session cannot select among multiple groups.

`leaveMesh()` leaves through the native node, clears the mesh, its ID, and its peers from state, and offers a fresh ticket so a member can enroll the device again. Its notice reports whether every remaining device was notified; notified devices relay the departure, and unreached devices can learn it when they contact the departed device while it retains that mesh's copy (up to 64 departed meshes). A later scan founds a new mesh, like a fresh receiver. Post-leave ticket refresh is cancellable when the node suspends, but native `SpiritNode.pair()` blocks inside `Dispatchers.IO`: offline teardown can still wait for its bounded request deadline (about 10 seconds). A failed leave keeps the membership and reports `Could not leave mesh`. See [leaving and rejoining](../wiki/design/nodes.md#leaving-and-rejoining).
